...
export class RegisterAccountComponent {
  ...
  registerCompte(nomUtilisateur: string, motDePasse: string, role: string): void {
    if (this.data.etudiant && this.data.etudiant._id) { // Check if etudiant and _id are defined
      this.etudiantService.addCompteToEtudiant(this.data.etudiant._id, nomUtilisateur, motDePasse, role='candidat').subscribe(
        () => {
